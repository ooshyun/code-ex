...
struct GraphNode {
  ...
  friend std::ostream& operator<<(std::ostream& os, const GraphNode& g) {
    os << g.id;
    return os;
  }
  explicit GraphNode(int id = -1) {
    this->id = id;
  }
  ~GraphNode(){}
};
...
class Graph {
 private:
  bool hasPathDFS(GraphNode *source,
                 GraphNode *destination,
                 std::vector<int> visited) {
    visited.push_back(source->id);

    std::cout << "Visited " << source->id << std::endl;
    for (std::vector<int>::iterator it = visited.begin();
        it != visited.end(); ++it)
      std::cout << " " << *it << " ";
    std::cout << std::endl;
    ...
    // for (std::vector<int>::iterator it = source->adjacent.begin() ;
    //  it != source->adjacent.end(); ++it)
    //     std::cout << " " << *it << " ";
    // std::cout << std::endl;
  ...
  bool hasPathBFS(GraphNode *source, GraphNode *destination) {
    ...
    while (!visit_nodes_next.empty()) {
      ...
      if (std::find(visited.begin(), visited.end(), idbuf) != visited.end())
        continue;
      ...
      for (std::vector<int>::iterator it = bufnode->adjacent.begin();
         it != bufnode->adjacent.end(); ++it)
        visit_nodes_next.push(*it);
    }
 ...
 public:
  ...
  bool hasPathDFS(int source, int destination) {
    ...
    // for (std::vector<int>::iterator it = s->adjacent.begin();
      // it != s->adjacent.end(); ++it)
    //     std::cout << " " << *it+1 << " ";
    // std::cout << std::endl;

    // std::cout << "Node " << d->id << std::endl;
    // for (std::vector<int>::iterator it = d->adjacent.begin();
      // it != d->adjacent.end(); ++it)
    //     std::cout << " " << *it+1 << " ";
    // std::cout << std::endl;
...
void testGraph()
{
  ...
  for (i = 0; i < num_node; i++) {
    ...
    for (std::vector<int>::iterator it = bufGraphNode->adjacent.begin();
          it != bufGraphNode->adjacent.end(); ++it)
      std::cout << " " << *it << " ";
    std::cout << "\n";
  }
  ...
  std::cout << "DFS PATH? \n";
  if (solution.hasPathDFS(source-1, destination-1))
  ...
  std::cout << "BFS PATH? \n";
  if (solution.hasPathBFS(source-1, destination-1))
    std::cout << "yes" << std::endl;
  else
    std::cout << "no" << std::endl;
  std::cout << "--------------\n";
