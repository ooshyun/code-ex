...
void worker(vector<int>::iterator start,
...
void worker_race_condition(int &counter) {  // NOLINT
    for (int i = 0; i < 10000; i++) {
        counter++;
    }
}
...
void worker_mutex (int &counter, std::mutex &m) {  // NOLINT
    for (int i = 0; i < 10000; i++) {
...
void worker_dead_lock(int &counter, std::mutex &m) {  // NOLINT
    for (int i = 0; i < 10000; i++) {
        m.lock();  // critial section start
        counter++;
    }
...
void worker_lock_guard(int &counter, std::mutex &m) {  // NOLINT
    for (int i = 0; i < 10000; i++) {
...
void worker1_deadlock_2(std::mutex &m1, std::mutex &m2) {  // NOLINT
    for (int i = 0; i < 5; i++) {
...
void worker2_deadlock_2(std::mutex &m1, std::mutex &m2) {  // NOLINT
    for (int i = 0; i < 5; i++) {
        std::lock_guard<std::mutex> lock2(m2);
        std::lock_guard<std::mutex> lock1(m1);
        // Do something
...
void worker1_deadlock_escape(std::mutex &m1, std::mutex &m2) {  // NOLINT
    for (int i = 0; i < 5; i++) {
        m1.lock();
        m2.lock();
...
void worker2_deadlock_escape(std::mutex &m1, std::mutex &m2) {  // NOLINT
    for (int i = 0; i < 5; i++) {
        while (true) {
            m2.lock();
...
void _test_thread_escape_deadlock(void (*func_worker1)
                                    (std::mutex &, std::mutex &),
                                void (*func_worker2)
                                    (std::mutex &, std::mutex &)) {
...
void test_thread(void) {
    ...
    // _test_thread_escape_deadlock(worker1_deadlock_escape,
    //                             worker2_deadlock_escape);
}